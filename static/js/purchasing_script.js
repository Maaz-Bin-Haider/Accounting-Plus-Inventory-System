...
function downloadInvoicePDF() {
  ...
  doc.setFont("helvetica","bold");
  doc.setFontSize(16);
  doc.text("Generated by Financee Accounting System", PL, 14);
  doc.setFontSize(11);
  doc.text("PURCHASE INVOICE", PL, 23);
  doc.setFont("helvetica","normal");
  doc.setFontSize(8.5);
  doc.text(`Invoice #: ${invoiceId||"DRAFT"}`, PR, 14, {align:"right"});
  doc.text(`Date: ${purchDate||new Date().toLocaleDateString()}`, PR, 22, {align:"right"});
  ...
  // Keep the divider below the metadata text so it cannot strike through the
  // vendor name (the text at y=30 extends above and below its baseline).
  doc.setDrawColor(147,197,253);
  doc.setLineWidth(0.6);
  doc.line(PL,36,PR,36);
  ...
  doc.setTextColor(150,150,150);
  doc.setFont("helvetica","normal");
  doc.text("Developed by Maaz Rehan", W/2, 290, {align:"center"});
  doc.text(`Printed on ${new Date().toLocaleString()}`, W/2, 294, {align:"center"});
