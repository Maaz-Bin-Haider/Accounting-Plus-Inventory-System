...
-- ============================================================================
-- FIX 10: get_serial_number_details returned ONE ROW PER SoldUnits record.
-- A serial that went through more than one sale cycle (purchase -> sale ->
-- sale-return -> sale again) has multiple SoldUnits rows: the old 'Returned'
-- one and the current 'Sold' one. The function LEFT JOINed SoldUnits with no
-- filter and no ordering, so it emitted both rows in arbitrary (heap) order.
--
-- Every caller reads the FIRST row (fetchone / item[0]), so the sale-return
-- lookup (saleReturn.sale_return_lookup) and the sale-return serial
-- validation showed the PREVIOUS sale's price and customer instead of the
-- current one. Reported production case: serial SH0YLM37J01TV showed 1600.00
-- (old, already-returned sale) instead of its live 1.00 sale.
--
-- Fix: collapse SoldUnits to a single authoritative row per unit via a
-- LATERAL pick that prefers the active 'Sold' record, then the most recent
-- by sold_unit_id. The function now returns exactly one row per serial and
-- every caller sees the current sale. Column list and behaviour for
-- never-sold serials (NULL sale columns, 'In Stock' status) are unchanged.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_serial_number_details(serial text)
RETURNS TABLE(serial_number character varying, item_name character varying, brand character varying, category character varying, purchase_invoice_id bigint, vendor_name character varying, purchase_date date, purchase_price numeric, in_stock boolean, sales_invoice_id bigint, customer_name character varying, sale_date date, sold_price numeric, current_status character varying)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
    ...
    -- Pick ONE SoldUnits row: the active 'Sold' record wins, otherwise the
    -- most recent sale. This keeps the function single-row per serial.
    LEFT JOIN LATERAL (
        SELECT su2.*
        FROM SoldUnits su2
